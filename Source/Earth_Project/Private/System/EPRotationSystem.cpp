...
{
    return GetWorld() ? GetWorld()->GetAuthGameMode<AEPGameMode>() : nullptr;
}

void AEPRotationSystem::OnTimeChanged(float Seconds)
{
    UpdateSunRotation();
    UpdateMoonRotation();
}
...
void AEPRotationSystem::UpdateSunRotation()
{
    ...
    const float DayNewYaw = FMath::GetMappedRangeValueClamped(TRange<float>{0.0f, static_cast<float>(HoursInDay)},    //
                                                              TRange<float>{0.0f, 360.0f},                            //
                                                              GameMode->GetCurrentHour());
    // Angle for sun simplified yearly movements.
    const float YearNewYaw = FMath::GetMappedRangeValueClamped(TRange<float>{0.0f, static_cast<float>(DaysInYear)},    //
                                                               TRange<float>{360.0f, 0.0f},                            //
                                                               GameMode->GetCurrentDay());
    const float DayNewPitch = FMath::Sin(FMath::DegreesToRadians(YearNewYaw)) * MaxSunAngle;
...
void AEPRotationSystem::UpdateMoonRotation()
{
    ...
    const float NewYaw = FMath::GetMappedRangeValueClamped(TRange<float>{0.0f, static_cast<float>(HoursInDay)},    //
                                                           TRange<float>{EndOfCircle, BeginOfCircle},              //
                                                           GameMode->GetCurrentHour());
    const float NewPitch = FMath::Sin(FMath::DegreesToRadians(NewYaw)) * MaxMoonAngle;
